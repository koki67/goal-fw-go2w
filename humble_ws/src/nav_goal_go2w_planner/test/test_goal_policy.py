...
class TestPureFunctions:
    ...
    def test_parse_goal_update_strategy_accepts_known_values(self):
        assert parse_goal_update_strategy("queue") == GoalUpdateStrategy.QUEUE
        assert parse_goal_update_strategy(" PREEMPT ") == GoalUpdateStrategy.PREEMPT

    def test_parse_goal_update_strategy_rejects_invalid_values(self):
        with pytest.raises(ValueError, match="invalid goal_update_strategy"):
            parse_goal_update_strategy("latest")
...
class TestGoalPolicy:
    ...
    def test_preempt_strategy_requests_preempt_when_active(self):
        p = GoalPolicy(min_update_distance=0.5, update_strategy="preempt")
        p.offer(_g(0, 0), can_dispatch=True)
        d = p.offer(_g(5, 5), can_dispatch=True)
        assert d.action == Action.PREEMPT
        assert d.reason == "preempt_active"
        assert p.active == _g(0, 0)
        assert p.pending == _g(5, 5)

    def test_preempt_strategy_keeps_duplicate_active_suppression(self):
        p = GoalPolicy(min_update_distance=0.5, update_strategy="preempt")
        p.offer(_g(0, 0), can_dispatch=True)
        d = p.offer(_g(0.1, 0.1), can_dispatch=True)
        assert d.action == Action.NONE
    ...
    def test_preempt_strategy_keeps_duplicate_pending_suppression(self):
        p = GoalPolicy(min_update_distance=0.5, update_strategy="preempt")
        p.offer(_g(0, 0), can_dispatch=True)
        p.offer(_g(5, 5), can_dispatch=True)
        d = p.offer(_g(5.1, 5.1), can_dispatch=True)
        assert d.action == Action.NONE
        assert d.reason == "duplicate_pending"
        assert p.pending == _g(5, 5)

    def test_preempt_strategy_replaces_pending_with_newest_distinct_goal(self):
        p = GoalPolicy(min_update_distance=0.5, update_strategy="preempt")
        p.offer(_g(0, 0), can_dispatch=True)
        p.offer(_g(5, 5), can_dispatch=True)
        d = p.offer(_g(10, 10), can_dispatch=True)
