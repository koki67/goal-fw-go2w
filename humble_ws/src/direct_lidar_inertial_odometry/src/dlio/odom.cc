...
void dlio::OdomNode::getParams() {
  ...
  this->get_parameter("dlio/verbose", this->verbose_);
}
...
void dlio::OdomNode::callbackPointCloud(const sensor_msgs::msg::PointCloud2::SharedPtr pc) {
  ...
  if (this->keyframes.size() == 0) {
    ...
    // Publish the initial odom->base_link TF immediately so slam_toolbox (and
    // any other TF consumer) can process the very first scan without waiting a
    // full scan period for the second callbackPointCloud invocation.
    geometry_msgs::msg::TransformStamped initial_tf;
    initial_tf.header.stamp    = this->imu_stamp;
    initial_tf.header.frame_id = this->odom_frame;
    initial_tf.child_frame_id  = this->baselink_frame;
    initial_tf.transform.translation.x = this->state.p[0];
    initial_tf.transform.translation.y = this->state.p[1];
    initial_tf.transform.translation.z = this->state.p[2];
    initial_tf.transform.rotation.w = this->state.q.w();
    initial_tf.transform.rotation.x = this->state.q.x();
    initial_tf.transform.rotation.y = this->state.q.y();
    initial_tf.transform.rotation.z = this->state.q.z();
