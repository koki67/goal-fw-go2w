#!/bin/bash
# Launch the nav-goal-go2w container with X11, CycloneDDS, host networking,
# and the host repository bind-mounted at /external for live config edits.
#
# Usage:
#   bash docker/run.sh
#   bash docker/run.sh --remote-viz
#   bash docker/run.sh --remote-viz --remote-viz-iface wlan0 ros2 launch nav_goal_go2w_bringup bringup.launch.py map:=/external/maps/office bridge_dry_run:=true
set -e
...
ROBOT_IFACE="${NAV_GOAL_ROBOT_IFACE:-eth0}"
REMOTE_VIZ_IFACE="${NAV_GOAL_REMOTE_IFACE:-wlan0}"
REMOTE_VIZ="${NAV_GOAL_REMOTE_VIZ:-false}"
CUDA="false"
ROBOT_IFACE_OVERRIDDEN=false
if [ -n "${NAV_GOAL_ROBOT_IFACE:-}" ]; then
...
Options:
  ...
  --remote-viz-iface IFACE     Wi-Fi/interface name used with --remote-viz
                               (default: wlan0 or NAV_GOAL_REMOTE_IFACE).
  --robot-iface IFACE          Robot/internal DDS interface (default: eth0 or
                               NAV_GOAL_ROBOT_IFACE).
  --cuda                       Use the CUDA image variant and pass --gpus all.
...
if [ "$CUDA" = "true" ] && [ -z "${NAV_GOAL_IMAGE:-}" ]; then
    IMAGE="${NAV_GOAL_CUDA_IMAGE:-nav-goal-go2w:cuda}"
fi
...
exec docker run -it --rm \
    ...
    --name="${NAV_GOAL_CONTAINER_NAME:-nav-goal-go2w}" \
    "$IMAGE" \
    "${CMD[@]}"
